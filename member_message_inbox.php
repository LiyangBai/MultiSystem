...
define('IN_TG',true);
//定义常量，用来指定本页的内容
define('SCRIPT','member_message_inbox');
//引入公共文件
...
if ($_GET['action']=='delete'&&isset($_POST['ids'])) {
	...
	if (_affected_rows()) {
		_close();
		_location('SUCCESS!','member_message_inbox.php');
	}else{
		_close();
		_alert_back('FAIL!');
	}
...
//分页
_page("SELECT tg_id FROM tg_message WHERE tg_touser='{$_COOKIE['username']}'",8);
//从数据库中获取数据结果集
$_result=_query("SELECT tg_id,tg_fromuser,tg_touser,tg_content,tg_state,tg_date FROM tg_message WHERE tg_touser='{$_COOKIE['username']}' ORDER BY tg_date DESC LIMIT $_pagenum,$_pagesize");
?>
<!DOCTYPE html>
<html lang="en">
<head>
		...
		<div id="member_main">
			...
			<form method="post" action="?action=delete">
				<table>
					...
					?>
					<tr><td><?php echo $_html['fromuser']; ?></td><td><a href="member_message_detail.php?id=<?php echo $_html['id']; ?>" title="<?php echo $_html['content']; ?>"><?php echo $_html['html_content']; ?></a></td><td><?php echo $_html['date']; ?></td><td><?php echo $_html['state']; ?></td><td><input type="checkbox" name="ids[]" value="<?php echo $_html['id']; ?>"></td></tr>	
					<?php }
